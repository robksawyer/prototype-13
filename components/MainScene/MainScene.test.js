...
describe('components', () => {
  describe('MainScene', () => {
    it('should mount with props', function () {
      const wrap = mount(<MainScene>Hello World</MainScene>)
    ...
    it('should render as type and with content', function () {
      const wrap = render(<MainScene>Hello World</MainScene>)

      expect(wrap[0].type).toEqual('tag')
      expect(wrap[0].name).toEqual('div')
      // contain styles
      expect(wrap[0].attribs.class).toContain('main_scene')
