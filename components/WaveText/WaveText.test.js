...
describe('components', () => {
  describe('WaveText', () => {
    it('should mount with props', function () {
      const wrap = mount(<WaveText>Hello World</WaveText>)
    ...
    it('should render as type and with content', function () {
      const wrap = render(<WaveText>Hello World</WaveText>)

      expect(wrap[0].type).toEqual('tag')
      expect(wrap[0].name).toEqual('div')
      // contain styles
      expect(wrap[0].attribs.class).toContain('wave_text')
